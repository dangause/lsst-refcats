...
from .htm import cones_to_htm as cones_to_htm_ids  # or cones_to_htm7 if you kept that name

def run_cones(ns: argparse.Namespace) -> None:
    # gather pointings
    if ns.fits_dir:
        pts = list(pointings_from_fits_dir(
            ns.fits_dir,
            ns.fits_recursive,
        ...
        pts = list(pointings_from_butler(
            ns.butler, ns.instrument, ns.include_calibs, normalize_where(ns.registry_where)
        ))
        if not pts:
    ...
    print(f"Unique pointings: {len(cones)} | radius={ns.radius_arcmin:.2f} arcmin | HTM depth={ns.depth}")
...
def main() -> None:
    ...
    cones = sub.add_parser("cones", help="Make cones.csv + htm7_list.txt from Butler/FITS/CSV/arrays")
    cones.add_argument("--fits-dir", default=None, help="Directory with FITS files to scan for WCS centers")
    cones.add_argument("--fits-recursive", action="store_true", help="Recurse into subdirectories")
    cones.add_argument("--fits-include", default=None, help="Regex: only include FITS paths that match")
    cones.add_argument("--fits-exclude", default=None, help="Regex: exclude FITS paths that match")
    cones.add_argument("--save-used-fits", default=None, help="(reserved) write list of files used")
    cones.add_argument("--csv", default=None, help="CSV with columns ra,dec (degrees)")
    cones.add_argument("--butler", default=None, help="Local Butler repo to read visit.region centroids")
    ...
    cones.add_argument("--radius-arcmin", type=float, default=6.0, help="Cone radius in arcmin")
