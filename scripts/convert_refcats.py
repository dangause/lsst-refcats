...
def convert_one(name: str, out_base: Path, config: Path, source_csv: Path, force: bool) -> Path:
    """
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Convert fetched Gaia/PS1 CSVs into tiled refcats.")
    ap.add_argument("--out-root", default="data", help="Root directory for tiled outputs (default: data)")
    ap.add_argument("--refdate", default=None, help="YYYYMMDD for output dir names; default=UTC today")
    ap.add_argument("--force", action="store_true", help="Re-run conversion even if map exists")
    ap.add_argument("--only", choices=["gaia", "ps1"], default=None, help="Convert only one catalog")

    # Input CSVs (produced by your fetchers)
    ap.add_argument("--gaia-csv", default="./data/gaia_dr3_all_cones/gaia_dr3_all_cones.csv")
