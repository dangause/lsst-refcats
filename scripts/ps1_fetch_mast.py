...
def _maybe_load_pointings_from_butler(
    ...
    try:
        ...
        if getattr(e, "name", "") == "deprecated":
            raise SystemExit("Missing dependency: install the PyPI package 'Deprecated' "
...
def main():
    ...
    ap.add_argument("--mag-band", choices=["g","r","i","z","y"], default="r",
    ...
    ap.add_argument("--batch-size", type=int, default=BATCH_SIZE_DEFAULT, help="Cones per shard file")
    ap.add_argument("--sleep", type=float, default=SLEEP_BETWEEN_DEFAULT, help="Pause between HTTP requests (s)")
    ap.add_argument("--max-retries", type=int, default=MAX_RETRIES_DEFAULT, help="Max retries per cone")
    ...
    for k, i0 in enumerate(range(0, n, args.batch_size), start=1):
        ...
        for j, (ra, dec) in enumerate(zip(rr, dd), start=1):
            ...
            df = _fetch_one_cone(ra, dec, args.radius_arcmin, args.sleep, args.max_retries)
        ...
        df_batch = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(columns=PS1_COLUMNS)
