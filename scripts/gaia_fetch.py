...
def _unitvec_to_xyz(u) -> Tuple[float, float, float]:
    ...
    if hasattr(u, "getX"):
        ...
        x = getattr(u, "x", None); y = getattr(u, "y", None); z = getattr(u, "z", None)
...
def _pointings_from_visit_regions(butler, instrument: str, include_calibs: bool, registry_where: str | None):
    ...
    where_clause = f"{instrument_clause} AND ({registry_where})" if registry_where else instrument_clause
...
def load_pointings(args) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if args.fits_dir:
        roots = args.fits_dir if isinstance(args.fits_dir, (list, tuple)) else [args.fits_dir]
        ras_list, decs_list = [], []
        scanned = 0
        for root in roots:
            for ra, dec in pointings_from_fits_dir(root, recursive=args.fits_recursive, debug=args.debug_fits):
                ras_list.append(ra); decs_list.append(dec); scanned += 1
        if not ras_list:
            raise SystemExit(
    ...
    raise SystemExit("No pointings provided. Use --fits-dir (with optional --fits-recursive), --csv, --butler, or --ras/--decs.")
...
def main():
    ...
    ap.add_argument("--fits-recursive", action="store_true", help="Recurse into subdirectories of each --fits-dir.")
    ap.add_argument("--debug-fits", action="store_true", help="Verbose logging for FITS discovery and header parsing.")
    ...
    ap.add_argument("--g-min", type=float, default=None, help="Minimum Gaia G mag (inclusive) for TAP query")
    ap.add_argument("--g-max", type=float, default=None, help="Maximum Gaia G mag (inclusive) for TAP query")
    ...
    mag_desc = []
    if args.g_min is not None: mag_desc.append(f"G≥{args.g_min}")
    if args.g_max is not None: mag_desc.append(f"G≤{args.g_max}")
    mag_str = f" | mag cut: {' & '.join(mag_desc)}" if mag_desc else ""

    print(f"Unique pointings: {len(ras)} | radius={args.radius_deg:.5f} deg | batch={args.batch_size}{mag_str}")
    ...
    for k, (i, j, rr, dd, radii) in enumerate(make_batches(ras, decs, args.radius_deg, args.batch_size), start=1):
        ...
        df = _run_one_batch(
            up, cols_sql=COLS_SQL, max_retries=args.max_retries,
